...
const page = () => {
  ...
  const fetchBorrowCards = async () => {
    try {
      const response = await fetch(`https://library-backend-ydnf.onrender.com/borrow-card/`, {
        method: "GET",
      });
  ...
  const fetchExpired = async (list) => {
    try {
      ...
      const responses = await Promise.all(
        list.map((item) =>
          fetch(`https://library-backend-ydnf.onrender.com/borrow-card/expired/${item?.id}`, {
            method: "PUT",
          })
        )
      );
  ...
  const fetchMailing = async (list) => {
    try {
      ...
      const response = await fetch(
        "https://library-backend-ydnf.onrender.com/borrow-card/askToReturn",
        {
          method: "POST",
          headers: {
