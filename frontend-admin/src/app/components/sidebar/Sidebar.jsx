  ...
  return (
    <Button
      ...
      className={`w-full justify-start mb-3 cursor-pointer flex items-center text-[15px] ${
        isActive ? "text-black" : "text-[#A3AED0]"
      }`}
      onClick={handleClick}
    >
      <img
        ...
        className={`mr-3 w-5 h-6 ${
          isActive ? "filter brightness-0 invert-[10%]" : ""
        }`}
      />
      <span>{label}</span>
    </Button>
...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`fixed left-0 h-full w-52 p-4 transform transition-transform duration-200 ease-in-out md:translate-x-0 flex flex-col z-50 md:z-0 ${
        isSidebarOpen
