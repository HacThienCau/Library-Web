...
const page = () => {
  ...
  useEffect(() => {
    ...
    const fetchPage = async () => {
      try {
        ...
        const res = await fetch(`https://library-backend-ydnf.onrender.com/childrenOf/${data.id}`, {
          method: "GET",
        });
  ...
  const handleFind = async(bookId, status) => {
    try {
      if(status === "Đang mượn") {
        const response = await fetch(`https://library-backend-ydnf.onrender.com/bookInBorrowing/${bookId}`, {
          method: "GET",
        });
        if(!response.ok) {
        ...
        const response = await fetch(`https://library-backend-ydnf.onrender.com/bookInLost/${bookId}`, {
          method: "GET",
        });
        if(!response.ok) {
          toast.error(response.text());
