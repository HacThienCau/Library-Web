...
function page() {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        const response = await axios.get(`https://library-backend-ydnf.onrender.com/book/${id}`);
  ...
  useEffect(() => {
    const checkBookInCart = async () => {
      try {
        const res = await axios.get(`https://library-backend-ydnf.onrender.com/carts/user/${userId}`);
