...
Bởi tô màu là một hình thức chữa lành đơn giản mà hiệu quả, nên mỗi khi thấy bực dọc, chán nản, hay khiến mình bận rộn hơn hãy thử tìm tới “Tô bình yên - vẽ hạnh phúc” cùng hộp màu đã cất lâu trong tủ và... Sao không thể là một đám mây màu tím, một mái tóc vàng tươi hay một nụ hoa màu xanh biển nhỉ? Không cần phải đắn đo đâu, bạn cứ thoải mái xóa đi căng thẳng, xóa đi những gam màu u ám, tự tay mình điểm tô những màu sắc tươi sáng lấp lánh, đầy ắp hy vọng theo ý thích, tận hưởng những phút thư giãn thật bình yên không muộn phiền thôi nào.`,
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md mt-25">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-700 font-semibold">Tác giả: {book.author}</p>
          <p><span className="font-bold">Thể loại:</span> {book.category}</p>
          <p><span className="font-bold">NXB:</span> {book.publisher}</p>
          <p className="flex items-center gap-2 font-semibold">
            <CheckCircle className="text-green-500" /> Trạng thái: {book.status}
          </p>
          <p><span className="font-bold">Lượt mượn:</span> {book.borrow_count} lượt</p>
      ...
      {/* Thông tin chi tiết */}
      <div className="mt-6">
        <h2 className="text-lg font-bold bg-gray-200 p-2 rounded-lg">Thông tin chi tiết</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          <p><span className="font-semibold">Mã sách:</span> {book.book_code}</p>
          <p><span className="font-semibold">Kích thước:</span> {book.size}</p>
          <p><span className="font-semibold">Số trang:</span> {book.pages}</p>
          <p><span className="font-semibold">ISBN:</span> {book.isbn}</p>
          <p><span className="font-semibold">Năm xuất bản:</span> {book.publish_year}</p>
          <p><span className="font-semibold">Ngôn ngữ:</span> {book.language}</p>
        </div>
      </div>

      {/* Giới thiệu */}
      <div className="mt-6">
        <h2 className="text-lg font-bold bg-gray-200 p-2 rounded-lg">Giới thiệu</h2>
        <p className="mt-2 text-gray-800 leading-relaxed">{book.description}</p>
      </div>
      <BookReview />
    </div>
  );
