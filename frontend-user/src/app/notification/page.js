...
const NotiCard = ({ message, timestamp, read }) => {
  const dateObj = new Date(timestamp);

  const formattedDate = dateObj.toLocaleDateString("vi-VN");   // 22/04/2025
  const formattedTime = dateObj.toLocaleTimeString("vi-VN", {
    hour: "2-digit",
    minute: "2-digit",
  });  
  return (
    <article className="flex items-center gap-4 p-4 bg-white rounded-lg shadow-sm border hover:shadow-md transition cursor-pointer">
      {!read && <FaCircle className="text-red-500 text-xs" title="Chưa đọc" />}
